...
if __name__=='__main__':
    ...
    #                                 Subject(course_year=dic["year"], course_semester=dic["semester"], course_colgnm=dic["colgnm"], course_sustnm=dic["sustnm"], course_pobjnm=dic["pobtnm"], course_shyr=dic["shyr"], course_profnm=dic["profnm"], course_ltbdrm=dic["ltbdrm"], course_sbjtclss=dic["sbjtclss"], course_clssnm=dic["clssnm"], course_pnt=dic["pnt"], course_remk=dic["remk"]).save()
    category = {
        "00000" : ["21","32","33","34","35","36","65","66","67","68","69","92","93","91","99"],
        "3A100" : ["3A110", "3A120", "3A130", "3A132", "3A133", "3A134", "3A140", "3A142", "3A143", "3A144", "3A150", "3A160", "3A170", "3A180", "3A190", "3A200"]
    }
    
    newList = []
    test = Subject.objects.all()
    for item in test:
        tmp = {"year" : item.course_year, 
        "semester" : item.course_semester, 
        "colgnm" : item.course_colgnm, 
        "sustnm" : item.course_sustnm, 
        "pobtnm" : item.course_pobjnm, 
        "shyr" : item.course_shyr, 
        "profnm" : item.course_profnm, 
        "ltbdrm" : item.course_ltbdrm, 
        "sbjtclss": item.course_sbjtclss, 
        "clssnm": item.course_clssnm, 
        "pnt":item.course_pnt, 
        "remk":item.course_remk}
        newList.append(tmp)
